<#
  ...
  两个必须知道的点（脚本已自动处理，改脚本时别改回去）：
    * 编码后端默认 ffmpeg（x264 / H.264）。CLI 自己的默认是 opencv，
      会写成 MPEG-4 Part 2——二十年前的老编码，同画质下体积大得多。
    * SeedVR2 的 CLI 完全不处理音频，输出天然没有音轨。脚本跑完会用 ffmpeg
      把源音轨原样接回去（视频流 -c:v copy，不重编码）。
...
param(
  ...
  [switch]$TenBit,                 # 需要 ffmpeg + libx265，10bit 输出减少色带

  # 视频编码后端。默认 ffmpeg（x264，H.264）；
  # 用 opencv 会写成 MPEG-4 Part 2（老编码，同画质下体积大得多），只在没 ffmpeg 时才用。
  [ValidateSet('ffmpeg', 'opencv')][string]$VideoBackend = 'ffmpeg',
...
# 让 CLI 用上「能用的」ffmpeg：CLI 内部写死了裸命令 ffmpeg，只认 PATH，
# 所以这里把完整版目录提到 PATH 最前面（见 common.ps1 的 Enable-FfmpegOnPath）。
$FfmpegExe = $null
if ($VideoBackend -eq 'ffmpeg') {
...
Write-Detail "编码后端  : $VideoBackend$(if ($TenBit) { '（10bit / x265）' } else { '（x264 / H.264）' })"
if ($FfmpegExe) { Write-Detail "ffmpeg    : $FfmpegExe" }
...
# 目录模式的预检：CLI 是以「整个目录」为单位调用的，没法让它只挑几个文件跑，
# 所以必须在开跑前先看一遍——全都已存在就直接退出，别白算几小时。
# （只存在一部分时没法跳过那几个，只能全量重跑，这里给个提示。）
$pendingDirFiles = @()
if ($isDir) {
  foreach ($f in (Get-VideoFiles -Path $InputPath)) {
    $b = [IO.Path]::GetFileNameWithoutExtension($f.Name)
    if (-not (Test-Path (Join-Path $OutDir "${b}_${Resolution}p_seedvr2.mp4"))) { $pendingDirFiles += $f }
  }
  if ($pendingDirFiles.Count -eq 0 -and -not $Overwrite) {
    Write-Done "全部输出已存在，无需处理（加 -Overwrite 可强制重跑）"
    Write-Host "输出目录：$OutDir" -ForegroundColor Green
    exit 0
  }
  $totalDirFiles = @(Get-VideoFiles -Path $InputPath).Count
  if ($pendingDirFiles.Count -lt $totalDirFiles) {
    Write-Note "有 $($totalDirFiles - $pendingDirFiles.Count) 个文件已有输出，但 CLI 按目录调用无法跳过，本次会全部重跑"
  }
}
...
if ($code -ne 0) {
  ...
  Write-Detail '编码失败       -> 确认 ffmpeg 在 PATH 里；实在没有就加 -VideoBackend opencv（输出老编码 MPEG-4，体积大，不推荐）'
  exit $code
}

# ---------- 5. 收尾 ----------
# SeedVR2 的 CLI 只出画面、不处理音频，音轨得自己接回来（见 common.ps1 的 Restore-AudioTrack）
if (-not $isDir) {
  ...
  if ($new.Count -gt 0) {
    ...
    if ((Test-Path $target) -and -not $Overwrite) {
      ...
      [void](Restore-AudioTrack -Source $InputPath -Target $target)
  ...
  # 目录模式下 CLI 输出的文件名与输入同名（只把扩展名统一成 mp4）。
  # 这里统一改名加上 _<分辨率>p_seedvr2 后缀，原因有两个：
  #   1) 与方案一（_2x）、方案二（_4x_flashvsr）的命名保持一致，三个方案的产物能一眼区分；
  #   2) 避免「输入目录 == 输出目录」时，输出把源文件直接覆盖掉。
  $renamed = 0; $fixed = 0
  foreach ($srcFile in (Get-VideoFiles -Path $InputPath)) {
    $base = [IO.Path]::GetFileNameWithoutExtension($srcFile.Name)
    $raw  = Join-Path $OutDir "$base.mp4"
    $dst  = Join-Path $OutDir "${base}_${Resolution}p_seedvr2.mp4"
    if (-not (Test-Path $raw)) { Write-Note "没找到输出，跳过：$raw"; continue }
    if ((Test-Path $dst) -and -not $Overwrite) {
      Write-Note "输出已存在，跳过：$dst（加 -Overwrite 可覆盖）"
      continue
    }
    if ($raw -ne $dst) { Move-Item -Path $raw -Destination $dst -Force }
    $renamed++
    if (Restore-AudioTrack -Source $srcFile.FullName -Target $dst) { $fixed++ }
  }
  Write-Done ("处理完成，用时 {0:hh\:mm\:ss}" -f $sw.Elapsed)
  Write-Detail "已处理 $renamed 个文件"
  if ($fixed -gt 0) { Write-Detail "已为 $fixed 个输出回接音轨" }
}
